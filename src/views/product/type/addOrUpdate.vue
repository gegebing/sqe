<template>
  <el-dialog v-if="visible" :title="!dataForm.id ? $t('common.table.addBtn') : $t('common.table.editBtn')"
    ...
    <el-form :model="dataForm"
             ...
             label-width="100px">
      <!--上级分类-->
      <el-form-item :label="$t('productType.list.queryForm.parentName')"
                    prop="parentCode">
    ...
    methods: {
      init(id, parentCode) {
        ...
        if (this.dataForm.id) {
          ...
          $http.http({
            type: "post",
            url: "yqkjShopProductTypeDetail",
            data: {key: id}
          }).then((data) => {
            if (data.code == 200) {
              this.dataForm = data.body;
              this.dataForm.parentCode = data.body.value;
              var index = this.dataForm.parentCode.lastIndexOf("|");
              if (index != -1) {
                this.dataForm.parentCode = this.dataForm.parentCode.substring(0, index);
              } else {
      ...
      dataFormSubmit(){
        ...
        this.$refs['dataForm'].validate((valid,message) => {
            if (valid) {
              var url = "yqkjShopProductTypeAdd";
              if (this.dataForm.id) {
                url = "yqkjShopProductTypeEdit"
              }
              $http.http({
                type: "post",
                url: url,
