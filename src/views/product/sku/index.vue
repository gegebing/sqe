  ...
  <div class="app-container">
    <el-form :inline="true" :model="formInline" size="small">
      <el-form-item :label="$t('shopSku.list.queryForm.name')">
        <el-input v-model="formInline.name" :placeholder="$t('shopSku.list.queryFormTip.name')" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="fetchData" icon="el-icon-search">{{$t('common.table.queryBtn')}}</el-button>
        <auth-button type="primary" perms="product_sku_add" :label="$t('common.table.addBtn')" @click="addOrEdit(0)" icon="el-icon-circle-plus"/>
      </el-form-item>
    </el-form>
    <el-table
      :data="list"
      ...
      <el-table-column :label="$t('shopSku.list.table.name')" width="260" align="center">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('common.table.op')"   width="350"  align="center">
        <template slot-scope="scope">
          <auth-button type="primary" perms="product_sku_edit"  size="mini"  :label="$t('common.table.editBtn')"   @click="addOrEdit(scope.row)"/>
          <auth-button type="primary" perms="shop_sku_language_manage"  size="mini"  :label="$t('common.table.lan')"   @click="language(scope.row.id, scope.row.skuCode)"/>
          <auth-button type="primary" v-if="!scope.row.status" perms="product_sku_disable"  size="mini"  :label="$t('common.table.able')"   @click="switchStatus(scope.row.id, scope.row.status)"/>
          <auth-button type="primary" v-if="scope.row.status" perms="product_sku_able"  size="mini"  :label="$t('common.table.disAble')"   @click="switchStatus(scope.row.id, scope.row.status)"/>
          <auth-button type="primary" perms="product_sku_add" :label="$t('common.table.addBtn')"  size="mini" @click="addOrEdit(0, scope.row.value)" icon="el-icon-circle-plus"/>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  methods: {
    switchStatus(id, status){
      var url = "yqkjShopProductSkuDisable";
      if (!status) {
        url = "yqkjShopProductSkuAble";
      }
      $http.http({
        type: "post",
        url: url,
    ...
    language(id, skuCode){
      ...
      this.$nextTick(() => {
        this.$refs.language.init(id, skuCode);
      });
    },
    addOrEdit(data, parentCode) {
      //新增
