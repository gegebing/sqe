  ...
  <div class="app-container">
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-form-item label="角色编码">
        <el-input v-model="formInline.code" placeholder="角色编码" clearable></el-input>
      </el-form-item>
      <el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      udpateConfigAuth() {
        $http.http({
          type: "post",
          url: "upms_role_updateroleconfigauth",
          data: {
      ...
      toConfigAuth(roleId) {
        ...
        $http.http({
          ...
          if (data.body) {
            ...
            $http.http({
              type: "post",
              url: "upms_role_getroleconfigauth",
              data: {
                key: roleId
              }
            }).then((data) => {
      ...
      switchStatusM(){
        var url = "upms_role_able";
        if (this.formDialog.status) {
          url = "upms_role_disable";
        }
        $http.http({
          type: "post",
          url: url,
      ...
      opData(){
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            var url = "upms_role_add";
            if (this.formDialog.id) {
              url = "upms_role_udpate";
            }
            $http.http({
              type: "post",
              url: url,
              data: {
                roleCode: this.formDialog.code,
                roleName: this.formDialog.name,
      ...
      toEdit(data){
        ...
        this.formDialog.name = data.roleName;
        this.formDialog.code = data.roleCode;
      },
      toAdd(){
        this.resetForm('formDialog');
        this.dialogVisible = true;
